...
def normalize_similarity(sim_matrix: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:

    row_sum = sim_matrix.sum(dim=1, keepdim=True) + eps
    return sim_matrix / row_sum
...
def torch_z_normalized(x, dim=1):
...
def jaccard_coef(tensor: torch.Tensor):
    ...
    intersection = torch.matmul(tensor, tensor.T)


    row_sums = tensor.sum(dim=1).view(-1, 1)
...
def translate_result(tensor: torch.Tensor or np.ndarray):
    ...
    elif isinstance(tensor, (np.ndarray, pd.Series, pd.DataFrame)):
        data_np = np.asarray(tensor)
    else:

        data_np = np.asarray(tensor)

    flattened_data = data_np.ravel()

    numeric_series = pd.to_numeric(pd.Series(flattened_data), errors='coerce')

    final_data_np = numeric_series.fillna(0.0).values.astype(np.float32)

    return pd.Series(final_data_np)
...
def f1_score_binary_efficient(true_data, predict_data, step=0.01):
    thresholds = torch.arange(0.0, 1.0 + step, step, device=true_data.device)
    best_f1 = 0.0
    best_thresh = 0.5  # fallback

    for thresh in thresholds:
        preds = (predict_data >= thresh).float()
        tp = torch.sum(preds * true_data)
        fp = torch.sum(preds * (1 - true_data))
        fn = torch.sum((1 - preds) * true_data)
        denom = tp * 2 + fp + fn
